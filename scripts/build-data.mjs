...
// --- "Rest of the World": UN World total minus the kept 120, per year ---
// Using the World aggregate (not just the sum of named countries) means this
// also captures small territories without ISO codes, so the bubbles add up to
// the real UN world figure — true ~100% coverage.
function worldTotals(file, estCol, projCol) {
  const { header, rows } = parseRows(file);
  const iEst = header.indexOf(estCol);
  const iProj = header.indexOf(projCol);
  const out = {};
  for (const r of rows) {
    if (r[0] !== 'World') continue;
    const year = +r[2];
    if (!YEARS.includes(year)) continue;
    const raw = r[iEst] !== '' && r[iEst] != null ? r[iEst] : r[iProj];
    if (raw === '' || raw == null) continue;
    out[year] = +raw / 1e6;
  }
  return out;
}
const worldBirths = worldTotals('births_raw.csv',
  'births__sex_all__age_all__variant_estimates',
  'births__sex_all__age_all__variant_medium__projected');
const worldPop = worldTotals('pop_raw.csv',
  'population__sex_all__age_all__variant_estimates',
  'population__sex_all__age_all__variant_medium__projected');

const restBirths = YEARS.map((y, i) =>
  +Math.max(0, (worldBirths[y] || 0) - kept.reduce((s, c) => s + c.births[i], 0)).toFixed(3));
const restPop = YEARS.map((y, i) =>
  +Math.max(0, (worldPop[y] || 0) - kept.reduce((s, c) => s + c.pop[i], 0)).toFixed(2));
const restCount = records.length - TOP; // named smaller countries folded in

const rest = {
  name: 'Rest of the World', code: 'ROW', iso2: '', continent: 'Other',
  flag: '🌍', isRest: true, count: restCount, births: restBirths, pop: restPop,
};
...
const emitted = [...kept, rest];
const body = emitted.map((c) => {
  const b = `[${c.births.join(', ')}]`;
  const p = `[${c.pop.join(', ')}]`;
  const extra = c.isRest ? `isRest: true, count: ${c.count}, ` : '';
  return `  { name: ${JSON.stringify(c.name)}, code: '${c.code}', iso2: '${c.iso2}', continent: ${JSON.stringify(c.continent)}, flag: '${c.flag}', ${extra}births: ${b}, pop: ${p} },`;
}).join('\n');
